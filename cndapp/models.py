...
class UserProfile(models.Model):
    ...
    dob_min_precision = models.ForeignKey(DOBPrecision, related_name='+', default=1,
                                          verbose_name='DOB Minimum Precision')
...
class Patient(models.Model):
    ...
    created_by = models.ForeignKey(User, related_name='patient_created_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    updated_by = models.ForeignKey(User, related_name='patient_updated_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    ...
    def outcome_due(self):
        ...
        for window in range(window_step, window_last, window_step):
            if self.visual_acuity_date + relativedelta.relativedelta(
                    months=window - 1) <= today and self.visual_acuity_date + relativedelta.relativedelta(
                    months=window + 1) >= today:
                outcomes = self.outcome_set.filter(
                    date__gte=self.visual_acuity_date + relativedelta.relativedelta(months=window - 1),
                    date__lte=self.visual_acuity_date + relativedelta.relativedelta(months=window + 1))
                if not outcomes:
                    return True
        return False
    ...
    visual_acuity_scale = models.ForeignKey(VisualAcuityScale, related_name='patient_visualacuity_scale',
                                            verbose_name='Scale')
    ...
    visual_acuity_fixation_preference = models.IntegerField(choices=FIXATION_CHOICES,
                                                            verbose_name='Fixation Preference', blank=True, null=True)
    ...
    iop_agents_right = models.ManyToManyField(IOPAgent, related_name='patient_iop_agents_right',
                                              verbose_name='Right IOP Agents')
    iop_agents_left = models.ManyToManyField(IOPAgent, related_name='patient_iop_agents_left',
                                             verbose_name='Left IOP Agents')
...
class Management(models.Model):
    created_by = models.ForeignKey(User, related_name='management_created_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    updated_by = models.ForeignKey(User, related_name='management_updated_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    agents = models.ManyToManyField(IOPAgent, verbose_name='IOP Agents', blank=True,
                                    limit_choices_to={'no_agents': False})
    comments = models.TextField(blank=True)
...
class Outcome(models.Model):
    created_by = models.ForeignKey(User, related_name='outcome_created_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    updated_by = models.ForeignKey(User, related_name='outcome_updated_set', blank=True, null=True,
                                   on_delete=models.SET_NULL)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    date = models.DateField()
    ...
    iop_agents_right = models.ManyToManyField(IOPAgent, related_name='outcome_iop_agents_right',
                                              verbose_name='IOP Agents Right')
    iop_agents_left = models.ManyToManyField(IOPAgent, related_name='outcome_iop_agents_left',
                                             verbose_name='IOP Agents Left')
    tonometry = models.ForeignKey(Tonometry)
    eua = models.ForeignKey(Anaesthesia, verbose_name='EUA')
    visual_acuity_method = models.ForeignKey(VisualAcuityMethod)
    visual_acuity_method_comment = models.TextField(verbose_name='Method Comment', blank=True)
    visual_acuity_scale = models.ForeignKey(VisualAcuityScale, related_name='outcome_visualacuity_scale',
                                            verbose_name='Scale')
